...
class Conv: public Layer {
 ...
 public:
  Conv(int channel_in, int height_in, int width_in, int channel_out,
       ...
       int pad_h = 0, bool usingDevice = false, bool usingOpt = false) :
       ...
       width_kernel(width_kernel), stride(stride), pad_w(pad_w), pad_h(pad_h), usingDevice(usingDevice), usingOpt(usingOpt)
  { init(); }
